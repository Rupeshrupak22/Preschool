      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              {courses.length > 0 ? `${courses.length} active courses this semester` : "No courses enrolled yet"}
            </p>
          </div>
        ...
        {courses.length === 0 ? (
          <div className="rounded-2xl border border-slate-100 bg-white p-10 text-center shadow-sm">
            ...
            <p className="mt-3 text-sm font-semibold text-slate-500">No courses enrolled</p>
            <p className="mt-1 text-xs text-slate-400">Your school admin will assign courses to you</p>
          </div>
        ) : (
                ...
                <div className="p-4">
                  ...
                  {course.topics.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {course.topics.map((t) => (
                        <span key={t} className="rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600">{t}</span>
                      ))}
                    </div>
                  )}
                  <a href="/student-dashboard/recorded-classes" className={`flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r ${course.color} py-2.5 text-xs font-black text-white shadow-sm transition hover:-translate-y-0.5`}>
