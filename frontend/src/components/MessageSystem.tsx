...
export function MessageSystem({ userEmail, userRole, recipients = [] }: MessageSystemProps) {
  ...
  return (
    <>
      {/* Notification Bell */}
      <button
        onClick={() => { setShowInbox(!showInbox); setShowCompose(false); }}
        className="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-100"
        title="Messages"
      >
      ...
      {/* Inbox Dropdown */}
      {showInbox && (
        <div className="fixed right-6 top-20 z-50 w-80 rounded-lg border border-slate-200 bg-white shadow-xl">
      ...
      {userRole !== "student" && (
        <div className="fixed bottom-6 right-6 z-50">
          {!showCompose ? (
            <button
              onClick={() => { setShowCompose(true); setShowInbox(false); }}
