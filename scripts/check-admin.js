...
async function main() {
  const email = process.argv[2] || process.env.ADMIN_EMAIL || "admin@adyapan.com";
  const config = dbConfig();
  const connection = await mysql.createConnection(config);

  const [admins] = await connection.query(
    `SELECT id, name, email, phone, role, signup_source, otp_verified, created_at, updated_at
     FROM users
  ...
  const [events] = await connection.query(
    `SELECT email, role, source, status, created_at
     FROM login_events
     WHERE email = ?
  ...
  console.log(JSON.stringify({ admin: admins[0] || null, recentLoginEvents: events }, null, 2));
}
